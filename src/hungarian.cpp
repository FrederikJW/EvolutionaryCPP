// copied from https://github.com/hellozhilu/MDMCP/blob/main/code/hungarian.cpp
...
void hungarian_solve(hungarian_problem_t* p) {
  ...
  // Begin Hungarian algorithm 18
	...
	for (k = 0; k < m; k++) {
		for (l = 0; l < n; l++) {
			if (p->cost[k][l] < row_dec[k] - col_inc[l]) exit(0);
		}
	}
	for (k = 0; k < m; k++) {
		l = col_mate[k];
		if (l < 0 || p->cost[k][l] != row_dec[k] - col_inc[l]) exit(0);
	}
	k = 0;
	for (l = 0; l < n; l++) {
		if (col_inc[l]) {
